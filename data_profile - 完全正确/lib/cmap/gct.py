...
class GCT(object):
    ...
    '''
    def __init__(self,src=None,read=False,verbose=True,cid=None,rid=None,col_inds=None, row_inds=None, matrix_only=False,frame=True):
        self.src = src
        self.version = ''
        self.matrix = ''
    ...
    def _read_gct(self,src,verbose=True,frame=True):
        ...
        '''
        if verbose:
            print('GCT_READER: reading .gct file...')
        ...
        if frame:
            self.frame = pd.DataFrame(self.matrix, index = self.get_row_meta('id'), columns = self.get_column_meta('id'))
    ...
    def _read_gctx(self,src,verbose=True,cid=None,rid=None, col_inds=None, row_inds=None, frame=True, convert_to_double=False):
        '''
    ...
    def read_gctx_matrix(self,src=None,cid=None,rid=None,col_inds=None, row_inds=None, verbose=True, convert_to_double=False, row_optimized=False):
        '''
        read just the matrix data from a gctx file
        '''
        if verbose:
            print('GCTX_READER: reading matrix data...')
        ...
        # a comment by omega: NO NEED TO SORTING RETRIEVED DATA!!!!
    ...
    def read_gctx_col_meta(self,src,col_inds=None, verbose=True):
        ...
        '''
        if verbose:
            print('GCTX_READER: reading column meta data...')
    ...
    def read_gctx_row_meta(self,src,row_inds=None, verbose=True):
        ...
        '''
        if verbose:
            print('GCTX_READER: reading row meta data...')
    ...
    def read(self,src=None,verbose=True,cid=None,rid=None, col_inds=None, row_inds=None, matrix_only=False, frame=True, convert_to_double=False):
        '''
        ...
        try:
            if extension == '.gct':
                ...
                if matrix_only:
                    ...
                    self._read_gctx(src,verbose=verbose,cid=cid,rid=rid,col_inds=col_inds,row_inds=row_inds, frame=frame)
            else:
                raise GCTException("source file must be .gct or .gctx")
        except GCTException as (instance):
            print (instance.message)

    def build(self, matrix, rid, cid, rdesc = None, cdesc = None, version = 'GCTX1.0', src = None):
        '''
    ...
    def build_from_DataFrame(self, frame, rdesc = None, cdesc = None, verbose = True, **kwargs):
        '''
